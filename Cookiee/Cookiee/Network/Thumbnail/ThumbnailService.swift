...
class ThumbnailService {
    ...
    func putThumbnail(thumbnailId: String, newThumbnail: Data, completion: @escaping (Result<ThumbnailResponseDTO, Error>) -> Void) {
        provider.request(.putThumbnail(userId: userId, thumbnailId: thumbnailId, newThumbnail: newThumbnail)) { result in
            switch result {
            case .success(let response):
                do {
                    let response = try JSONDecoder().decode(ThumbnailResponseDTO.self, from: response.data)
                    ...
                    completion(.failure(error))
                    print("putThumbnail Decoding error:", error)
                }
            case .failure(let error):
                completion(.failure(error))
                print("putThumbnail error:", error.errorDescription as Any)
            }
        }
    }
}
